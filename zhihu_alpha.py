...
#	    print type(int(number_up_vote[i].string))
    ...
    def getTopicFollower(self):
    	...
    	number_of_relevant_topic_follower = number_of_relevant_topic_follower_doc.find_all('strong')[1].string
    	print 'How many people follow the relevant topics: ', number_of_relevant_topic_follower

    # Get the number of people who follows the people answering the question
    def get_number_people_follow_answerer(self):
        soup = self.soup
        ...
        if number_answer == 0:
            ...
            for i in range(0, number_answer):
                try:
                    ...
                    person_soup = BeautifulSoup(response.content)
                    funs = person_soup.find('div', class_ = 'zm-profile-side-following zg-clear').find_all('a')[1].strong.string
                    funs = int(funs)
                    total_fun += funs
                    print 'How many followers of this people?', total_fun
                except:
                    None
            print ' How many people are following the people who answer this question?', total_fun
...
#            print soup.title
            ...
            for fun in funs:
                ...
                print fun_number
                total_funs += fun_number
                print 'How many people are following the people who follows this question?', total_funs
        except:
            followers_link = None
...
#       self.getFunsAnswerer()    
        self.get_number_people_follow_answerer()
